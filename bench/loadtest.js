...
// ─── 自定义指标 ───────────────────────────────────────────────
const messageSendDuration = new Trend("message_send_duration", true);   // POST /message 响应时间
const streamTTFB          = new Trend("stream_ttfb", true);             // SSE 首个 delta 延迟
const streamTotalDuration = new Trend("stream_total_duration", true);   // SSE 完整流时长
const streamEventCount    = new Trend("stream_event_count");            // 每次流的事件数
const messageErrors       = new Rate("message_errors");                 // POST /message 错误率
const streamErrors        = new Rate("stream_errors");                  // SSE 流错误率
const completedChats      = new Counter("completed_chats");             // 成功完成的完整聊天数

// ─── 配置 ─────────────────────────────────────────────────────
const BASE_URL    = __ENV.BASE_URL || "http://localhost:9899";
const SSE_TIMEOUT = parseInt(__ENV.SSE_TIMEOUT || "120");  // SSE 超时秒数
...
// ─── k6 运行选项（可被 run.sh 通过环境变量覆盖）──────────────
const vus      = parseInt(__ENV.VUS || "10");
const duration = __ENV.DURATION || "60s";
const rampUp   = __ENV.RAMP_UP === "true";

export const options = rampUp
  ? {
      stages: [
        { duration: "10s", target: Math.ceil(vus * 0.3) },
        { duration: "20s", target: Math.ceil(vus * 0.7) },
        { duration: "10s", target: vus },
        { duration: duration, target: vus },
        { duration: "10s", target: 0 },
      ],
      thresholds: {
        message_errors:        ["rate<0.1"],
        stream_errors:         ["rate<0.1"],
...
export default function () {
  ...
  sse.open(sseUrl, {
    ...
    client.on("event", function (event) {
      ...
      if ((Date.now() - sseStart) > SSE_TIMEOUT * 1000) {
        console.warn(`SSE timeout [vu${vuId}] after ${SSE_TIMEOUT}s`);
        client.close();
        return;
      }
  ...
  // 间隔 0.5-2s 再发下一轮，模拟用户思考时间
  sleep(0.5 + Math.random() * 1.5);
}

// ─── 测试结束汇总 ─────────────────────────────────────────────
export function handleSummary(data) {
  // 如果设置了 SUMMARY_EXPORT 环境变量，同时导出 JSON
  const result = {
